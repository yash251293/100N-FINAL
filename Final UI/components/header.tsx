...
import { Menu, Bell, Briefcase, Globe, Inbox, LayoutDashboard, MessageSquare, Users, Building2, Settings, LogOut } from "lucide-react"; // Added Menu, LogOut
...
export default function Header() {
  ...
  return (
    <header className="border-b bg-background relative"> {/* Added relative for mobile menu positioning */}
      <div className="flex items-center justify-between px-6 py-3">
        <div className="flex items-center space-x-4"> {/* Container for burger and logo */}
          {/* Hamburger Menu Button - Only for mobile */}
          {user && (
            <button
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="md:hidden p-2 rounded-md text-slate-600 hover:bg-slate-100 hover:text-primary"
        ...
        {/* Desktop Navigation & User Actions Container */}
        <div className="flex items-center space-x-1">
          {/* Desktop Navigation - Show only if user is logged in */}
          {user && (
          ...
          <div className="flex items-center space-x-1">
            {isLoading ? (
              ...
              <>
                {/* Login/Signup buttons if needed */}
              </>
            )}
          </div>
        </div>
      ...
      {/* Mobile Navigation Menu - Shown below header */}
      {user && isMobileMenuOpen && (
        <nav className="md:hidden absolute top-full left-0 right-0 bg-background shadow-lg py-2 z-50">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
