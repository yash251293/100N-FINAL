...
router.put('/profile/complete', authMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    if (userType === 'individual') {
      ...
      if (body.experiences !== undefined) updates.experiences = body.experiences; else updates.experiences = null;
      if (body.education !== undefined) updates.education_entries = body.education; else updates.education_entries = null;
      if (body.skills !== undefined) updates.skills = body.skills; else updates.skills = null;
      if (body.interests !== undefined) updates.interests = body.interests; else updates.interests = null;
      ...
      if (body.values !== undefined) updates.company_values = body.values; else updates.company_values = null;
      if (body.jobOpenings !== undefined) updates.job_openings = body.jobOpenings; else updates.job_openings = null;
      if (body.teamSize !== undefined) updates.team_size = body.teamSize; else updates.team_size = null;
      if (body.companyCulture !== undefined) updates.company_culture_summary = body.companyCulture; else updates.company_culture_summary = null;
      if (body.benefits !== undefined) updates.benefits_perks = body.benefits; else updates.benefits_perks = null;
    }

    // Check if any relevant data was actually provided for the given user type
    let relevantDataProvided = false;
    const individualFields = ['professionalSummary', 'experiences', 'education', 'skills', 'interests', 'portfolio', 'linkedin', 'github', 'website'];
    const companyFields = ['mission', 'vision', 'values', 'jobOpenings', 'teamSize', 'companyCulture', 'benefits'];

    if (userType === 'individual') {
      // Check if at least one of the individual-specific fields was present in the request body
      if (individualFields.some(field => body[field] !== undefined)) {
        relevantDataProvided = true;
      }
    } else if (userType === 'company') {
      // Check if at least one of the company-specific fields was present in the request body
      if (companyFields.some(field => body[field] !== undefined)) {
        relevantDataProvided = true;
      }
    }
    ...
    // Use 'updates' object which has all fields correctly mapped (including those set to null if key was present)
    const columns = Object.keys(updates);
    // If after mapping, columns is empty, it means none of the expected fields for that user type were in body.
    // This check is important if the initial body was empty or contained no relevant fields.
    if (columns.length === 0) {
        return res.status(400).json({ message: 'No valid fields to update for this user type.' });
    }
    ...
    const { rows } = await db.query(upsertQuery, [userId, ...queryValues]);
    ...
    console.error('Error completing profile route:', error); // Log specifically for this route
    next(error);
  }
});
